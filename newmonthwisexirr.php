...
function difference_rate($rate, $date_objects, $cashflows) {
    ...
    for ($i = 0; $i < count($date_objects); $i++) {
        $sum += $cashflows[$i] / pow(1 + $rate, ($date_objects[$i]->diff($date_objects[0])->days) / 365);
    }
    return $sum;
}
...
foreach ($date_objects as $i => $date) {
    $rate_guess = 0.1;
    $xirr = newton([$rate_guess, $date_objects, array_slice($cashflows, $i)], 'difference_rate', 'difference_rate_prime');
    $xirr_values[] = $xirr;
}
