<?php
function calculate_xirr($cashflows, $dates, $nav_values, $current_nav) {
    ...
    function difference_rate($rate) use ($date_objects, $cashflows) {
        $sum = 0;
    ...
    for ($i = 0; $i < count($date_objects); $i++) {
        $cashflows_subset = array_slice($cashflows, 0, $i + 1);
        $xirr = newton($initial_guess, 'difference_rate', 'difference_rate_prime');
        $xirr_values[] = $xirr;
    }
...
$xirr_values = calculate_xirr($cashflows, $dates, $nav_values, $current_nav);
