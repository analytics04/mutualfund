...
if (isset($_GET['sid'])) {
    ...
    $sid = $_GET['sid'];
    $sql = "
SELECT nav_value FROM (SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2004 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2005 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2006 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2007 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2008 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2009 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2010 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2011 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2012 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2013 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2014 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2015 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2016 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2017 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2018 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2019 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2020 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2021 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2022 where scheme_id = $sid 
UNION ALL
SELECT DATE_FORMAT(nav_date, '%Y-%m') AS month, nav_value FROM nav_2023 where scheme_id = $sid) as nav_data GROUP BY month ORDER BY month DESC;
";
    ...
    foreach ($result as $key => $value) {
        array_push($monthlyNavArray, $value[0]);
    }

    // $set_research_time_start = microtime(true);
    // echo "<pre>";
    // echo (set_reaSearch($d));
    // echo "</pre>";
    // $set_research_time_end = microtime(true);
    // $set_research_time = $set_research_time_end - $set_research_time_start;
    // echo "Time taken by set_research() is " . (float) $set_research_time . "<br>";
}
...
function set_reaSearch($sid)
{
    ...
    if ($currentNav['nav_value'] != "-1") {
        ...
        $get_nav_time_start = microtime(true);
        ...
        $get_nav_time_end = microtime(true);
        $get_nav_time = $get_nav_time_end - $get_nav_time_start;
        echo "Time taken by get_nav() and value allocation is " . (float) $get_nav_time . "<br>";

        $res['performanceRatios'] = performanceRatios($sid, $currentNav);
        $ph_time_start = microtime(true);
        $res['historicReturn'] = historicReturn($sid);
        $ph_time_end = microtime(true);
        $ph_time = $ph_time_end - $ph_time_start;
        echo "Time taken by Historic and Performance Ratio is " . (float) $ph_time . "<br>";


        $sip_time_start = microtime(true);
        $sip_time_end = microtime(true);
        $sip_time = $sip_time_end - $sip_time_start;
        echo "Time taken by sip_cal() and value allocation is " . (float) $sip_time . "<br>";
    } else {
        $res['status'] = false;
    ...
    //?for SIP calculation 
    // $res['sip_return'] = calculateDoubleTripleMonths($currentNav, $sid, $startDate);
...
function performanceRatios($sid, $currentNav)
{
    ...
    //?for monthe wise
    // for ($i = 1; $i < 37; $i++) {
    //     if (!get_nav_in($sid, $currentNav, 30 * $i)) {
    //         break;
    //     }
    //     array_push($preNavRtn, get_nav_in($sid, $currentNav, 30 * $i));
    // }
...
function sip_double($monthlyNavArray)
{
    ...
    for ($i = 0; $i < count($monthlyNavArray); $i++) {
        if (($total_investment * 2) < $totla_current_val) {
            return (int) $year . " Year " . (int)$months . " Month and Row is " . $month;
        }
        $total_investment += $investment_monthly;
        // echo "Total Investment is " . $total_investment . "<br>";
        $totla_current_val += $monthlyNavArray[0] * ($investment_monthly / $monthlyNavArray[$i]);
        // echo "Total current value is " . $totla_current_val . "<br>";
        $month++;
        $year = (int)$month / 12;
        $months = (int)$month % 12;
    }
    // echo "Number of cycle is " . $i . "<br>";
    return "-";
}
echo  sip_double($monthlyNavArray) == -1 ? "Not Yet" : "It take  " . sip_double($monthlyNavArray);
// echo  date("Y-m-d", strtotime(sip_double($monthlyNavArray) . " month"));;
