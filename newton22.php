<?php
function newton($initial_guess, $func, $func_prime, $date_objects, $cashflows, $max_iterations = 1000, $tolerance = 1e-6) {
    $guess = $initial_guess;

    for ($i = 0; $i < $max_iterations; $i++) {
        $f_value = $func($guess, $date_objects, $cashflows);
        $f_prime_value = $func_prime($guess, $date_objects, $cashflows);
...
// Calculate an initial guess based on the known data
$initial_guess = (array_sum($cashflows) / array_sum($nav_values)) / ($date_objects[count($date_objects) - 1]->diff($date_objects[0])->days / 365);

$xirr_values = [];
foreach ($date_objects as $i => $date) {
    $xirr = newton($initial_guess, 'difference_rate', 'difference_rate_prime', $date_objects, array_slice($cashflows, 0, $i + 1));
    $xirr_values[] = $xirr;
}
